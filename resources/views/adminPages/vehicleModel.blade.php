                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="card card-primary">
                            ...
                            <!-- form start -->
                            @if ($selectSubCatId == null)
                            <form role="form" action="/vehicle-model" method="post" enctype="multipart/form-data">
                                @else
                                    <form role="form" action="/edit-vehicle-model" method="post"
                            ...
                            <div class="card-body">
                                <div class="row">
                                            ...
                                            <select class="form-control select2" id="brandId" name="brandId" style="width: 100%;">
                                                ...
                                                @foreach ($allcategories as $category)
                                                    <option value="{{ $category->id }}" {{ $category->id == $editId ? 'selected' : '' }}>
                                                       {{$category->vehicleBrand->name }} - {{ $category->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                    ...
                                    <div class="form-group col-md-4">
                                        <label for="exampleInputEmail1">Vehicle Model Name <span
                                    ...
                                    <div class="form-group col-md-4">
                                        <label for="exampleInputEmail1">Vehicle Model launch Year <span
                                                style="color: red;">*</span></label>
                                        <input type="number" min="0" class="form-control" placeholder="Year"
                                            @if ($selectSubCatId != null) value="{{ $selectSubCatId->year }}" @endif
                                            name="year" required>
                                    </div>

                                    <div class="form-group">
                                        <label for="customFile">Select Model Image</label>
                                    ...
                                    <tbody>
                                        ...
                                        @for ($i = 0; $i < count($allSubcategories); $i++)
                                            <tr>
                                                ...
                                                <td>{{$allSubcategories[$i]->brandName }} - {{ $allSubcategories[$i]->vehicleCategory->name }}</td>
                                                <td>{{ $allSubcategories[$i]['name'] }}</td>
                                                <td>{{ $allSubcategories[$i]['year'] }}</td>
                                                <td> <!-- HTML code to open image in modal -->
<a href="{{ url($allSubcategories[$i]['image']) }}" target="_blank" >Preview</a>
</td>

                                                <td>
                                                <a href="/edit-vehicle-model/{{ $allSubcategories[$i]['id'] }}"
                                                        class="btn btn-primary btn-sm">Edit</a>
                                                    <a href="/delete-vehicle-model/{{ $allSubcategories[$i]['id'] }}"
